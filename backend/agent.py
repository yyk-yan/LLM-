...
def _extract(chunk, headers, req, llm_cfg: dict | None = None):
    ...
    usr_msg = (
        ...
        "- Unit stripping rule: if a field name already contains a unit (e.g. '金额(元)', '重量(kg)', '面积（㎡）'), "
        "and the extracted value carries the same unit, strip the unit from the value and output the bare number. "
        "Examples: field '金额(元)', value '1000元' -> '1000'; field '重量(kg)', value '5kg' -> '5'; "
        "field '面积（㎡）', value '30㎡' -> '30'. If the units differ, keep the value as-is.\n"
...
def _extract_with_cot(chunk, headers, req, llm_cfg: dict | None = None):
    ...
    usr_msg_extract = (
        ...
        "- Unit stripping rule: if a field name already contains a unit (e.g. '金额(元)', '重量(kg)', '面积（㎡）'), "
        "and the extracted value carries the same unit, strip the unit from the value and output the bare number. "
        "Examples: field '金额(元)', value '1000元' -> '1000'; field '重量(kg)', value '5kg' -> '5'. If the units differ, keep the value as-is.\n"
