...
**方式 A（改版后的，直接登陆之后，在主界面设置）：每用户登录后在 Web 界面里配置**
...
1. 用初始超级管理员账号（用户名：admin，密码admin123）登录
2. 进入「管理」面板可创建新租户、创建邀请码
3. 把邀请码发给同事，让其在登录页的「使用邀请码注册」标签页注册（邮箱格式要求：abcd@example.com）
